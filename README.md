...
The future scope of this application includes integrating a **real Google Maps API** (via `react-native-maps`) to replace the simulated 2D route map with actual live GPS routing, geofencing, and precise destination ETA tracking. Furthermore, we will implement a **persistent cloud database backend** (e.g., via Firebase or PostgreSQL). Currently, user data (order histories, favorites, cart) is stored locally via `AsyncStorage` and gets wiped if the app is deleted. Introducing a central backend will ensure that logging back in with the same credentials on any fresh installation successfully restores the user's historical profile and active cart. Additionally, we plan to build bidirectional chat synchronization between the Customer and Delivery/Retailer portals using a real-time WebSocket backend (e.g., Socket.io), integrate voice-activated accessibility features, and establish dedicated inventory forecasting dashboards for retailers.
